...
export default function CartPage() {
  ...
  const loadItems = (sorting = "") => {
    // Fonksiyon, önbellekteki ürünlerin listesi için bir dizi oluşturur ve bu dizinin elemanlarını, ürünlerin anahtarı ve miktarını birleştirerek birleştirir
    ...
    const uri = `${API_BASE_URL}/cart?ids=${queryString}&sorting=${sorting}`; // Rest API'ye istek gönderir
    appToast.showToast(true); // bildirim
    fetch(uri)
      .then((res) => res.json())
      .then((items) => {
        setItems(items);
  ...
  const filteredCartItems = items // filtreleme, ürünlerin adında bir anahtar kelime olarak verilen kelimeyi içerip içermediğine göre yapılır.
